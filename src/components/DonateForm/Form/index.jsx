...
const Form = ({getCardNumberProps, getExpiryDateProps, getCVCProps}) => {
  ...
  const transition = useTransition({isSubmitting, isSubmittedSuccessfully, paymentId, errorReason}, {
    from: {
      y: '10%', opacity: 0.0
    }, enter: {
      y: '0%', opacity: 1.0
  ...
  const processPayment = useCallback(async (paymentMethod, payload) => {
    try {
      let paymentResult;
  ...
  const onClickPayButton = useCallback(() => {
    if (!dirty || !isValid) {
      setTouched({
        name: true, email: true, amount: true, cardNumber: true, expiryDate: true, cvc: true
      }, true);
      return;
    }
  ...
  const onClickAppleOrGoogleButtonHandler = useCallback(event => {
    const isValid = Object.keys(errors).every(errorFieldKey => !['name', 'email', 'amount'].includes(errorFieldKey));
    if (!dirty) {
      event.preventDefault();
      setTouched({
        name: true, email: true, amount: true, cardNumber: false, expiryDate: false, cvc: false
      }, true);
    } else {
      const isCardDataErrorExist = Object.keys(errors).some(errorFieldKey => ['cardNumber', 'expiryDate', 'cvc'].includes(errorFieldKey));
  ...
  const onLoadGooglePaymentData = useCallback((paymentData) => {
    const payload = {
      formData: values,
      paymentData
    }
  ...
  const onLoadApplePaymentData = useCallback((paymentData) => {
    const payload = {
      formData: values,
      paymentData
    }
  ...
  const handleOnFieldFocus = useCallback(e => {
  ...
  return (<div className={style.container}>
      {transition((springStyle, {isSubmitting: isSubmitting2, isSubmittedSuccessfully, paymentId, errorReason}) => {
        if (isSubmitting2) {
          return (<animated.div style={springStyle} className={style.animatedContentWrapper}>
            <Loader/>
          </animated.div>);
            ...
            <FormikForm className={style.form}>
              ...
              <div className={style.fieldsWrapper}>
                ...
                <div className={style.fieldWrapper}>
                  ...
                  <ErrorMessage name="name">
                    {errorI18Key => (<AnimatedErrorMessage className={style.errorMessage}>
              ...
              <div className={style.buttonWrapper}>
                <div className={style.applePayAndGooglePayButtonsGroup}>
                  <ApplePayButton donationAmount={values.amount || 0} onClick={onClickAppleOrGoogleButtonHandler}
                                  onLoadPaymentData={onLoadApplePaymentData}/>
                  <GooglePayButton donationAmount={values.amount || 0} onClick={onClickAppleOrGoogleButtonHandler}
                                   onLoadPaymentData={onLoadGooglePaymentData}/>
                </div>
                <Button className={style.button} color="red" disabled={isSubmitting} tabIndex={0}
          ...
          return (<animated.div style={springStyle} className={style.animatedContentWrapper}>
            <PaymentResultNotification paymentId={paymentId} onClickOk={backToFormHandler}
                                       paymentStatus={PaymentResultNotification.paymentStatuses.SUCCESS}/>
          </animated.div>);
        } else {
          return (<animated.div style={springStyle} className={style.animatedContentWrapper}>
            <PaymentResultNotification paymentId={paymentId} secondaryInfo={errorReason}
